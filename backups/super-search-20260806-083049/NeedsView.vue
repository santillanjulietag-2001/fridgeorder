...
const allNeedsSelected = computed(() => {
  if (!needs.items.length) return false;
  const ids = new Set(needs.selectedIds.map(String));
  return needs.items.every((i) => ids.has(String(i._id)));
});
...
<template>
  ...
  >
          ...
          <div v-show="isCategoryOpen(cat)" class="category-body">
            <div v-for="item in needs.grouped[cat]" :key="item._id" class="option-row need-row">
              ...
              <div class="option-actions">
                <div class="price">{{ euro(item.estimatedPrice) }}</div>
                <button
                  class="btn-x"
                  type="button"
                  title="Quitar"
    ...
    >
      ...
      >
        <div class="results-header">
          <h3 id="product-detail-title">Detalle web</h3>
          <button
            class="btn-x"
            type="button"
            title="Cerrar"
        ...
        <div class="product-detail-body">
          ...
          <dl class="product-detail-meta">
            ...
            <div>
              <dt>Origen datos</dt>
              <dd>Datos en vivo de la web del súper</dd>
            </div>
          </dl>
      ...
      <div class="needs-float-main">
        ...
        <div class="needs-float-actions" @click.stop>
          ...
          <button
            type="button"
            class="btn ghost needs-float-sel"
            :class="{ active: needs.selectedIds.length === 0 }"
            :disabled="!needs.items.length"
